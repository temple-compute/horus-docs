...
The one exception is a [value artifact](/sdk/core/artifact#value-artifacts),
which resolves to its **authored value** rather than its path — see
[Value artifacts render their value](#value-artifacts-render-their-value).
...
<Callout type="warn" title="Ids must be valid identifiers everywhere">
...
<Callout type="info" title={<><code>$$</code> emits a literal <code>$</code></>}>
...
<Callout type="info" title={<>Shell <code>$VAR</code> is preserved</>}>
...
<Callout type="info" title={<><code>{'{'}{'}'}</code> passes through untouched</>}>
...
<Callout type="info" title="Reserved id">
...
## Value artifacts render their value

A [value artifact](/sdk/core/artifact#value-artifacts) — `NumberArtifact`,
`BooleanArtifact`, `StringArtifact`, or your own `ValueArtifact` subclass —
resolves `$id` to its authored `value`, not to a path. A task consuming
`$samples` gets `10`, not the path of a file whose contents happen to be `10`:

```python
samples = NumberArtifact(id="samples", path="samples.json", value=10)

CommandRuntime(command="python train.py --samples $samples")
# python train.py --samples 10
```

The value still materializes to a file, so anything that wants to read it
directly still can — `${samples.path}` gives you that path as usual. This only
changes how the bare placeholder renders.

Booleans render lowercased (`true` / `false`) to match the on-disk JSON, so
`$flag` and `cat ${flag.path}` agree.

<Callout type="warn" title={<><code>$id</code> is no longer a path here</>}>

`cat $flag` renders `cat true`, not a path. A command that needs the file must
say `${flag.path}` explicitly. `${flag.value}` is also available, but only on
value artifacts — the dotted form forwards straight to the artifact, so
`${result.value}` on a `FileArtifact` is an error, not a fallback.
...
See [Runtime System](/sdk/core/runtime) for how runtimes prepare their payloads.
