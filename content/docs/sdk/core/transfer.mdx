...
[`path_on_target`](/sdk/core/target) on the destination and `transfer()` returns
...
the source (via [`ArtifactStore`](/sdk/core/artifact#artifactstore)), streams the
...
For more details, refer to the [Auto-Registry documentation](/sdk/plugin-system/autoregistry) and the
[Auto-Registry Product documentation](/sdk/plugin-system/auto-registry-product).
