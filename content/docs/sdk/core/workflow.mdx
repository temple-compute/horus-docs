...
- `from_yaml()`: load and construct a workflow from a YAML file, recording the
  file it loaded as `source_path`
...
WorkflowEdge(
    ...
    target="assemble",
    target_input="kit",
    transfer=False,   # order assemble after worker, move no bytes
)
```
...
Capacity accounting lives in a single workflow-level placement manager,
exposed as the `placement` property. The scheduler and any composite task that
dispatches sub-tasks of its own (a `horus_map` awaiting its clones, for
instance) share that one manager, so the accounting for a given location stays
global across the whole run instead of resetting per dispatcher.
...
See [Transfer Strategy](/sdk/core/transfer) for details.

### `source_path`

`source_path` is the file `from_yaml()` loaded, resolved to an absolute path:

```python
wf = HorusWorkflow.from_yaml("my-pipeline/pipeline.yaml")
wf.source_path      # PosixPath('/abs/my-pipeline/pipeline.yaml')
```

It is `None` for a workflow built directly in Python or restored from a
snapshot, neither of which has a file behind it. Deriving it from
`_base_directory` would only work when the file happened to be named
`workflow.yaml`, and the CLI accepts any filename (`horus run pipeline.yaml`),
so the loader keeps the file path itself rather than making callers guess it.

Like `_base_directory`, it is runtime-only state, not a model field, and never
serialized: a snapshot's meaning is unchanged, and a workflow round-tripped
through one has `source_path is None`.
...
- **`expand(tasks=[...], edges=[...], artifacts=[...])`**: a transactional
  ...
  may reference tasks or artifacts added in the same batch. A batch also
  **supersedes** what it re-derives — see below.

#### `expand()` supersedes what a batch re-derives

`add_task` rejects a duplicate id. `expand` does not: a task in the batch whose
id already exists **replaces** the existing one, together with every edge that
touches that id, and a root artifact in the batch replaces an existing root of
the same id.

That asymmetry is deliberate, and it is what makes an expanding construct
re-runnable. A `horus_map` task registers its clones through `expand(...)`
every time it actually runs, so a run resumed from a snapshot that already
carries those clones would otherwise collide with itself:

```text
TaskIdsAreNotUniqueError: Multiple tasks share the same ID 'score[0]'
```

The freshly derived task has to win regardless of which is "newer", because its
paths are pinned under *this* run's root while the snapshot's copy still names
the previous run's directory.

Stale edges are dropped by **endpoint**, not by equality: an expander re-emits
every edge touching the tasks it owns, so any edge on a superseded id is one the
batch is about to re-state.

Duplicates *within* a single batch remain an error —
`TaskIdsAreNotUniqueError` for two tasks sharing an id, and
`ArtifactIdsAreNotUniqueError` for two root artifacts.

<Callout type="warn" title="Do not use expand() to update a task in place">

Supersession replaces the task object and every edge touching it. Passing a
partially-built task with the id of a live one silently drops that task's
existing wiring. `expand` is for re-deriving a batch you own end to end.
...
it runs: a map commits all of its clones in a single `expand(...)`, while a
loop injects the next iteration one step at a time.
