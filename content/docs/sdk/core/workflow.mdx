...
That asymmetry is deliberate, and it is what makes an expander re-runnable.
Every expander re-derives its whole expansion each time it is triggered
(`is_complete()` is hardcoded `False`), so a run resumed from a snapshot that
already carries a previous run's expansion would otherwise collide with itself:
