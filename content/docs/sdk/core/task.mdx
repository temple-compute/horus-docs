...
- `is_complete()`: **async**: return `True` when all output artifacts are present and valid; used to skip already-complete tasks when `skip_if_complete=True`. It is `async` because checking an output artifact may require a round-trip to a remote target (see [`ArtifactStore`](/sdk/core/artifact#artifactstore)); callers must `await` it.
...
class BaseTask(AutoRegistry, entry_point="task"):
    ...
    def working_dir(self) -> str:
        """Per-task, per-invocation folder under the target's working directory."""
        ...
...
When a task begins, Horus creates a fresh target-side working directory for
that invocation:

```text
<target working_directory>/<task id>/<execution id>
```

`execution id` is an opaque unique identifier. Re-running a task therefore
never reuses the prior job's current directory, preventing relative outputs,
uploaded scripts, side artifacts, and scheduler files from overwriting an
earlier execution. The directory is created before the executor runs, so the
same layout applies to local, remote, and scheduler-backed targets such as
Slurm.

It is created **inside** the task middleware chain, immediately before `_run()`
— not before the chain is entered. Every task middleware's `before` hook has
therefore already run by the time the directory exists, so a middleware may
repoint `task.target` and have the directory created at the new location. See
[Task Middleware](/sdk/plugin-system/middleware/domains#task-middleware).
...
[DAG planning](/sdk/core/workflow#dag-planning) and [edges](/sdk/core/workflow#edges).
...
See [Side Artifacts](/sdk/core/side-artifact) for the full guide.
...
- `kind_description: ClassVar[str]` — a longer description string. Prefer
    ...
    as `_`) for translations; see the [SDK i18n guide](/sdk/i18n), e.g.:
...
When a task does declare outputs, `HorusTask` also fingerprints its inputs and
its own configuration (runtime and executor) before running, and skips a
future run only while that fingerprint still matches. A runtime or executor
that reads a local file, such as `PythonScriptRuntime`'s script, reports it
through `local_files()` so its digest is folded into the fingerprint too;
editing the file invalidates the cache even though the path on disk did not
change. `BaseRuntime.local_files()` and `BaseExecutor.local_files()` both
default to `[]`.
...
See [FunctionTask](/sdk/core/function-task) for the full guide and examples.
