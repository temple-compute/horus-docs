...
[Detachable Execution](/sdk/core/detaching).
...
[`ArtifactStore`](/sdk/core/artifact#artifactstore) and the target-agnostic
[`GenericTransfer`](/sdk/core/transfer#generictransfer) build on to check, delete, and
...
- `working_directory`: base directory **on the target host** where per-task,
  per-invocation working directories are created. A task invocation uses
  `<working_directory>/<task id>/<execution id>`, so reruns do not overwrite a
  prior job's files. It is `str | None` and defaults to `None`; read it through
  `resolved_working_directory`, never directly
...
- `run_command` / `put_file` / `get_file` / `mkdir` / `list_dir`: the agentless
  ...
  [Detachable Execution](/sdk/core/detaching). `list_dir` enumerates a target-side
  directory, used to collect side artifacts back to the orchestrator (see
  [Side Artifacts](/sdk/core/side-artifact))
...
`horus_runtime`'s internal translator. See [SDK i18n guide](/sdk/i18n)
...
channel. See [Detachable Execution](/sdk/core/detaching).
