...
- **`working_dir`** — a per-task, per-invocation folder under its target's
  working directory where inputs are materialized and outputs are written. It
  has the form `<working_directory>/<task id>/<execution id>`, so reruns do not
  overwrite prior side products.
...
own [transfer strategies](/sdk/core/transfer) and are not pulled back wholesale.
...
executor. See [Targets](/sdk/core/target#targets-are-agentless-channels).
