...
description: Use horus package to zip a workflow and its referenced files so it can run on another machine.
---
...
```text
  - refine.idx.json (not found; assumed generated)
```

The reason is that not every input path exists before a run. An expanding
construct may materialize an input under the run directory as it grows the
graph: a conditional [loop](/guides/loops), for example, writes each
iteration's index file moments before the body clone that consumes it runs.
So Horus tells you what it left out and lets you judge whether it mattered.
