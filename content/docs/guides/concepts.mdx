...
A task is only skipped while its inputs and its own configuration are
unchanged since the run that produced its outputs. That includes local files
a runtime or executor reads directly, such as a `python_script` task's script:
editing the script invalidates the cache and the task re-runs, even though
