...
A task is marked `FAILED` whether it broke while running or during its setup —
acquiring its target, or transferring its inputs onto that target. Setup happens
before the task starts, so a task can be reported failed having never run: a
remote target that refuses the connection, for instance, fails the task that was
going to run there rather than the one that produced the inputs.
